// Parses the NED_PERMITS.KML file found in the data folder,
...
func ParseKML() ([][]Permit, error) {
   ...
   var all [][]Permit
   total := 0
   file := "/Users/sabra/go/src/clustering/kml/nedpermits/data/NED_PERMITS.kml"
      ...
      x := 0
		...
		//	fmt.Printf("- ExtendedData: %s = %s\n", sd.Name, sd.Value)
         ...
         if sd.Name == "st_length(shape)" {
            if pm.ObjectId == 34303 || pm.ObjectId == 28884 || pm.ObjectId == 31752 || pm.ObjectId == 33548 {
   ...
   fmt.Printf("Number of NED permits: %d\n", total)
